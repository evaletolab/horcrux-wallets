...
**0/5** First, we store one share part of Shamir that is useless without other parts. Seconds, the shared part is XOR mixed before to be stored. Third, we already planned a tools to delegate the storage and avoid the relation between one wallet and one Horcrux.
